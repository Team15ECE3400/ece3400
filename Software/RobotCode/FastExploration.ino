...
void loop() {
 ...
 while(analogRead(RightRear)<850 && analogRead(LeftRear)<850 ); // Move if rear sensors are at the intersection
  if (turnRight) {
...
void LineFollowing() {
  ...
  //drive servos
   ...
   if(!LFCrossed && !RFCrossed){
    left.write(Lforward);            
    right.write(Rforward);
    //Serial.println("both on either side of line");
   }
  ...
  if(LFCrossed && RFCrossed && CheckAgain ) { // Make a decision on whether to go left, right, or straight.
